...
internals.applyRoutes = function(server, next) {
  ...
  server.route({
    ...
    config: {
      ...
      pre: [AuthPlugin.preware.ensureAdminGroup('root')],
    },
    handler: function(request, reply) {
      const query = {};
      if (request.query.username) {
        query['user.name'] = new RegExp(
          '^.*?' + EscapeRegExp(request.query.username) + '.*$',
          'i'
        );
      }
    ...
    config: {
      ...
      pre: [
          ...
          method: function(request, reply) {
            User.findByUsername(request.payload.username, (err, user) => {
              ...
              if (
                ...
                return reply(
                  Boom.conflict(
                    'User is already linked to another admin. Unlink first.'
                  )
                );
              }
        ...
        },
        {
          ...
          method: function(request, reply) {
            if (
              request.pre.admin.user &&
              request.pre.admin.user.id !== request.pre.user._id.toString()
            ) {
              return reply(
                Boom.conflict(
                  'Admin is already linked to another user. Unlink first.'
                )
              );
            }
            ...
            const update = {
              $set: {
                'roles.admin': {
                  ...
                  name:
                    request.pre.admin.name.first +
                    ' ' +
                    request.pre.admin.name.last,
                },
              },
            };
