...
lab.before(done => {
  ...
  const proxy = {};
  proxy[Path.join(process.cwd(), './server/models/auth-attempt')] =
    stub.AuthAttempt;
...
lab.experiment('Login Plugin (Create Session)', () => {
  ...
  lab.test('it returns early when abuse is detected', done => {
    ...
    server.inject(request, response => {
      ...
      Code.expect(response.result.message).to.match(
        /maximum number of auth attempts reached/i
      );
  ...
  lab.test(
    'it returns an error when creating a new auth attempt fails',
    done => {
      stub.AuthAttempt.abuseDetected = function(ip, username, callback) {
        callback(null, false);
      };
  ...
  lab.test('it returns early after creating a new auth attempt', done => {
    ...
    server.inject(request, response => {
      Code.expect(response.statusCode).to.equal(400);
      Code.expect(response.result.message).to.match(
        /username and password combination not found/i
      );
...
lab.experiment('Login Plugin Forgot Password', () => {
  ...
  lab.test('it succussfully sends a reset password request', done => {
    ...
    server.plugins.mailer.sendEmail = function(
      options,
      template,
      context,
      callback
