...
lab.experiment('Modal', () => {
  ...
  lab.test(
    'it updates props to show/hide (for keyup listener and body class)',
    done => {
  ...
  lab.test(
    'it closes when the backdrop is clicked through modal, but not when clicking on the dialog',
    done => {
      const props = {
        onClose: function() {
          done();
  ...
  lab.test(
    'it closes when the esc key is pressed, but not another key',
    done => {
      const props = {
        onClose: function() {
          done();
