...
lab.experiment('ControlGroup', () => {
  lab.test('it renders', done => {
    ...
    const controlGroupTag = ReactTestUtils.findRenderedDOMComponentWithTag(
      controlGroup,
      'div'
    );
  ...
  lab.test('it renders without label or help elements', done => {
    ...
    const ControlGroupEl = React.createElement(ControlGroup, props);
    const controlGroup = ReactTestUtils.renderIntoDocument(ControlGroupEl);
    const labelEls = ReactTestUtils.scryRenderedDOMComponentsWithTag(
      controlGroup,
      'label'
    );
    const helpEls = ReactTestUtils.scryRenderedDOMComponentsWithClass(
      controlGroup,
      'help-block'
    );
