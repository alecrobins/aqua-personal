...
lab.experiment('Admin Admins User Reducer', () => {
  lab.test(
    'it handles a GET_DETAILS_RESPONSE action (only setting user if present)',
    done => {
  ...
  lab.test(
    'it handles a LINK_USER_RESPONSE action (only setting user if present)',
    done => {
      let state = Store.getState().user;
      const originalId = state.id;
      const originalName = state.name;
  ...
  lab.test(
    'it handles a UNLINK_USER_RESPONSE action (only resetting user if present)',
    done => {
      Store.dispatch({
