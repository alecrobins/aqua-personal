...
lab.experiment('Admin Accounts Status Reducer', () => {
  lab.test(
    'it handles a GET_DETAILS_RESPONSE action (only setting status if present)',
    done => {
      Store.dispatch({
  ...
  lab.test(
    'it handles a GET_STATUS_OPTIONS_RESPONSE action (only setting options if present)',
    done => {
      let state = Store.getState().status;
  ...
  lab.test(
    'it handles a NEW_STATUS_RESPONSE action (only setting status if present)',
    done => {
      Store.dispatch({
