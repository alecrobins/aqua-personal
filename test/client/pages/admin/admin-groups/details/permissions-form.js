...
const Form = Proxyquire(
  '../../../../../../client/pages/admin/admin-groups/details/permissions-form.jsx',
  {
...
lab.experiment('Admin Groups Permissions Form', () => {
  ...
  lab.test(
    'it handles adding a new permission (only when one is supplied, with a button click or enter key)',
    done => {
      const FormEl = React.createElement(Form, defaultProps);
      const form = ReactTestUtils.renderIntoDocument(FormEl);
  ...
  lab.test('it handles removing a permission', done => {
    ...
    const FormEl = React.createElement(Form, props);
    const form = ReactTestUtils.renderIntoDocument(FormEl);
    const button = ReactTestUtils.scryRenderedDOMComponentsWithClass(
      form,
      'btn-warning'
    )[0];
