...
lab.experiment('Parse Validation Helper', () => {
  ...
  lab.test(
    'it handles validation errors with keys (matching the because pattern)',
    done => {
      const data = {
        validation: {
          keys: ['name'],
  ...
  lab.test(
    'it handles validation errors with keys (matching nested because pattern)',
    done => {
      const data = {
        validation: {
          keys: ['name.first'],
        ...
        message:
          'name failed because [first failed because [first is required]]',
      };
      const result = ParseValidation(data);
